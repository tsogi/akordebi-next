...
export default function UploadInfo() {
  ...
  const UploadButton = ({ className = "" }) => (
    <button
      ...
      className={`w-full py-4 px-8 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 ${className}`}
    >
      რესურსის ატვირთვა
    </button>
  );
  ...
  const ResourceCard = ({ resourceData, isHighlighted = false }) => (
    <div className={`relative rounded-2xl overflow-hidden p-6 md:p-8 w-full flex flex-col shadow-lg border transition-all duration-300 hover:shadow-xl ${
      ...
      <div className="mb-6 relative">
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{resourceData.title}</h3>
        <div className="flex items-baseline mb-4">
          <span className="text-3xl font-bold text-green-600 dark:text-green-400">{resourceData.price}</span>
